...
/* Transformation code for Ze
 * def mavrosPoseCB(self, msg):
        ''' Handle mavros pose topic data '''
        with self.uavPoseLock:
            self.uavPose = msg
            
            # Update the orientation from the Imu topic
            if self.uavAtt is not None:
                with self.attLock:
                    self.uavPose.pose.orientation = self.uavAtt.orientation
        
        # Rename (for convenience) vehicle pose in EKF2 frame E
        p_ekf2 = msg.pose.position
        o_ekf2 = msg.pose.orientation
        
        # Get Rotation matrix from B frame to E frame
        quat_e = (o_ekf2.x, o_ekf2.y, o_ekf2.z, o_ekf2.w)
        invQ = tf.transformations.quaternion_inverse(quat_e)
        self.R_E_B = tf.transformations.quaternion_matrix(quat_e)
        self.R_B_E = tf.transformations.quaternion_matrix(invQ)
        
        # Position vector of body-fixed B frame in EKF2 frame E
        P_E_B = np.array([[p_ekf2.x], [p_ekf2.y], [p_ekf2.z], [1]])
    
        # Create transform from B frame to E frame and vice versa
        self.T_E_B = copy.deepcopy(self.R_E_B)
        self.T_E_B[0:3, 3:4] = P_E_B[0:3]
        self.T_B_E = np.linalg.pinv(self.T_E_B)
        
# Getting Euler angles...
    vo = self.uavPose.pose.orientation
            quat = (vo.x, vo.y, vo.z, vo.w)
...
#define ETH_IP        "128.46.186.214"    // IP Address of Desktop computer running Nexus
...
//#define DRONE_NAME    "BC_v2"           // Name of drone object in Vicon
//#define DRONE_NAME    "Intel_1"           // Name of drone object in Vicon
//#define DRONE_NAME    "Intel_2"           // Name of drone object in Vicon
#define DRONE_NAME    "Uvify_1"           // Name of drone object in Vicon
//#define DRONE_NAME    "Uvify_2"           // Name of drone object in Vicon
//#define DRONE_NAME    "IRIS_2_new"           // Name of drone object in Vicon

//export ROS_IP=192.168.1.54  # Wired

//#define VICON_TOPIC   "/intel1/mavros/fake_gps/mocap/pose"      // ROS Topic name: (/vicon/pose or /mavros/mocap/pose)
//#define VICON_TOPIC   "/intel1/mavros/vision_pose/pose"      // ROS Topic name: (/vicon/pose or /mavros/mocap/pose)
//#define VICON_TOPIC   "/intel2/mavros/vision_pose/pose"      // ROS Topic name: (/vicon/pose or /mavros/mocap/pose)
#define VICON_TOPIC   "/uvify1/mavros/vision_pose/pose"      // ROS Topic name: (/vicon/pose or /mavros/mocap/pose)
//#define VICON_TOPIC   "/uvify2/mavros/vision_pose/pose"      // ROS Topic name: (/vicon/pose or /mavros/mocap/pose)


//#define VICON_TOPIC   "/mavros/vision_pose/pose"
...
int main( int argc, char* argv[] )
{
  ...
  ros::NodeHandle n;
  bool tracking_drone = false;  // Flag to indicate drone is being tracked
  bool occluded_drone = false;  // Flag to indicate drone is occluded
  double warn_timer = time_keeper.now().toSec();

  // Advertise a vicon pose publisher with queue length = 100
  ros::Publisher vicon_pub = n.advertise<geometry_msgs::PoseStamped>(VICON_TOPIC,10);
  // *****************************************
  ...
  for(int i=0; i != 3 && ros::ok() ; ++i) // repeat to check disconnecting doesn't wreck next connect
  {
    ...
    while( !MyClient.IsConnected().Connected && ros::ok())
    {
      ...
      if(!ok)
      {
        ...
        ROS_INFO("Warning - connect failed...");
      }
    ...
    ROS_INFO("Client connected, waiting for new frames...");
    while( ros::ok() )
  #endif
    {
      // Get a frame
      ...
      while( MyClient.GetFrame().Result != Result::Success )
      {
        ...
        #else
          sleep(1);
          ROS_WARN("No frame received from Vicon Client! Trying again...");
        #endif
      ...
      tracking_drone = false;
      occluded_drone = false;
      ...
      for( unsigned int SubjectIndex = 0 ; SubjectIndex < SubjectCount ; ++SubjectIndex )
      {
        ...
        if(SubjectName.compare(DRONE_NAME) == 0) { tracking_drone = true; }
        ...
        for( unsigned int SegmentIndex = 0 ; SegmentIndex < SegmentCount ; ++SegmentIndex )
        {
         ...
         //                                              << _Output_GetSegmentStaticTranslation.Translation[ 1 ]  << ", " 
          ...
          if(SubjectName.compare(DRONE_NAME) == 0) {
      ...
      if(time_keeper.now().toSec() - warn_timer > 1.0/WARN_HZ) {
          if(!tracking_drone) { ROS_WARN("Not tracking %s!", DRONE_NAME); }
          else if(occluded_drone) {
              ROS_WARN("%s occluded!", DRONE_NAME);
          }
